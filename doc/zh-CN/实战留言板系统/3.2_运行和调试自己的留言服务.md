...
实际在网络中运行的 cyfs@bdt 协议并不会被 DEC App 的客户端和服务器直接使用，在开发者看来不管是前端还是后端，都只是处理有特殊Header的HTTP协议。这个设计让 cyfs@bdt 协议的实现细节对应用开发者透明，BDT能有机会进行持续迭代(BDT还是一个年轻的协议），同时也能降低开发者的学习和使用门槛。
...
TODO：要说一下AppManager是如何加载Servic的，尤其是docker镜像逻辑。

在运行 Service 之前，我先来深入了解一下 Service 启动程序。
...
1. 为请求路径 req_path 动态设置 access 权限，这里我们为每一个请求路径都只对 OwnerDec 开放全部的权限(Read/Write/Call)。
...
现在我们把服务发布到 OOD 并对服务的各项功能进行独立调试。
...
```shell
npx tsc //TODO:npx 是？还是老问题，尽量只用npm
```
...
TODO：增加一下DEC App打包发布的原理说明，尤其是对MetaChain的更新（应用发布一定会上链）

我们先 `打开命令行` ，然后在项目根目录下运行如下指令:
...
TODO：基于流程进一步说明AppManager的去中心化特性
...
测试发布新留言、修改留言以及删除留言后的 `root_state` 变化，可以参考下文的 `使用命令行工具查看 root_state` 去查看最新的 root_state

---

TODO：这里要说明一下测试程序的编写原理。
...
执行完之后，所有的测试脚本文件都在 deploy/src/service/test 文件下。
...
如果接口正常，控制台会打印`publish message msgKey is ${msgKey}, result: ${r}`，其中的 msgKey 就是新建留言对象的 key 值。
...
---
TODO：这里发布第一条留言后，可以用cyfs shell去查看RootState。这么写会连贯一点。而不是最后说
TODO：这里还要说一下Client端和Service端的日志查看，并对经典错误进行一些说明
...
打开 deploy/src/service/test/retrieve_message_test.js 文件，把上一步 `控制台` 打印出来的 `msgKey` 字符串赋值给 `main` 函数的 `msgKey` 常量。

修改好之后，在项目根目录下，打开终端执行如下指令：

```shell
node ./deploy/src/service/test/retrieve_message_test.js
```

如果接口正常，控制台会打印`retrieve message result: current Message key is ${msgRawObj.key}, content is ${msgRawObj.content}`，其中包含留言对象的 key 值和 content 内容。
...
打开 deploy/src/service/test/retrieve_message_test.js 文件，把在 `调试发布留言功能` 中 `控制台` 打印出来的 `msgKey` 字符串赋值给 `main` 函数的 `msgKey` 常量。此外，你也可以手动设置`content`常量的为你喜欢的任何字符串，目的是更改原有的留言内容。
...
如果接口正常，控制台会打印 `update message result: ${r}`。
...
打开 deploy/src/service/test/delete_message_test.js 文件，把在 `调试发布留言功能` 中 `控制台` 打印出来的 `msgKey` 字符串赋值给 `main` 函数的 `msgKey` 常量。
...
如果接口正常，控制台会打印`delete message result: ${r}`。
