...
//TODO:比较符合人的理解逻辑是index.html上要带peopleId参数（不带就是当前用户）。实现 以当前用户查看制定用户的留言板的效果
//     这意味着 总是在用自己的留言版 查看所有人的数据
//     基于这个讨论，会得出两种不同的link形式，但要有正确的权限，还是应该用a链接。（用o链接因为没有dec_id身份，可能会没有足够的权限）
//     现在的逻辑，似乎变成每发一条留言就会通知所有的好友了，这从产品上是不对的
...
```
//TODO:这个路由逻辑也逐步向 代理逻辑转换。即 “往别人拥有产权的列表里写对象，一定会把新对象放到自己的OOD上，然后再由OOD去发起跨Zone请求，这个逻辑的本质还是保存自己创建的一切对象”
