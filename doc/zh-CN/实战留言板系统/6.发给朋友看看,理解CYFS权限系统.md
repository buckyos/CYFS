...
1. 在留言服务的创建、修改和删除留言功能中增加通知好友同步的功能
2. 开放以上功能对应的请求路径的必要权限(access)
...
发布留言后跨 Zone 通知好友就是发送一个路径为`/messages/publish_req`的跨 Zone 请求，请求参数是新的留言对象。打开 src/service/routers/publish_message.ts 文件，放开以下代码的注释即可。
...
await stackWraper.postObject(msgObject, ResponseObjectDecoder, {
	...
	decId: stack.dec_id!,
	target: cyfs.PeopleId.from_base_58(peopleId).unwrap().object_id,
});
```
...
修改留言后跨 Zone 通知好友就是发送一个路径为`/messages/update_req`的跨 Zone 请求，请求参数也是新的留言对象。打开 src/service/routers/update_message.ts 文件，放开以下代码的注释即可。
...
删除留言后跨 Zone 通知好友就是发送一个路径为`/messages/delete_req`的跨 Zone 请求，请求参数是留言对象。打开 src/service/routers/delete_message.ts 文件，放开以下代码的注释即可。
...
## 再次发布一条留言试试能不能看到好友的留言

### 发布新代码到 OOD

经过上一节`在留言服务中增加通知好友的功能`的改造后，我们的留言板已经从`单机版`变成了`联机版`，现在可以发布新的代码到 OOD 实现 Dec App 的版本升级了。
在执行发布流程前，请打开根目录下的 cyfs.config.json 文件，修改`version`的版本号，比如由原来的`1.0.0`升级为`1.0.1`。修改完成后，重新发布更新后的代码到 OOD。

发布完成后，自己跟好友就可以在 CYFS 浏览器的`应用商店`中对留言板 App 进行升级安装。
...
1. Dec App 自己允许的权限。
   在 dec_ervice 中基于请求路径(req_path)的权限管理机制。也就是说，原则上，我们需要针对每一个 req_path 都配置对应的权限。
   为 req_path 配置权限，就是设置不同的 req_path 对应的 access-string。
2. Dec App 想要申请的权限。
   配置 acl.toml 文件，该文件会在 app 安装时，被 app-manager 读取，按照配置的各项值，向系统注册对应的权限。
...
目前根据 device 和 dec，从左到右分为六个组，每个分组都可以可以设置上一小节`权限分类`中的三个独立的权限：
...
顶层 key，为需要申请权限的 dec id，`self`是特殊的 key，代表自己的 dec id。
