...
为了带上前端页面，请打开根目录下的 cyfs.config.json 文件，修改`web`为：
...
把 `CYFS App Install Link` 对应的链接发给你的好友，根据`去 CYFS 浏览器中安装 Dec App 并查看`的步骤指引，让你的好友也成功安装上我们的留言板 Dec App！
...
1. 打开 src/common/constant.ts 文件，修改 `PEOPLE_IDS` 为`你自己的 peopleId` 以及`你好友的 peopleId`。
...
现在可以发布新的代码到 OOD 实现 Dec App 的版本升级了
