...
要做到这点，你必须在一个地方能设置好友的 peopleId，为了能尽可能简单的实现，我们将在页面的 URL 中的 search 中设置 target，target 值为好友的 peopleId。
总的来说，要实现这个功能，只需 2 步：
...
1. 修改 `queryMessageRecords` 方法，去除方法中第 2-3 行的代码注释。接着，修改调用 `listMessagesByPage` 方法的参数。这样，我们就可以指定要查看哪个好友的留言板(URL 中指定设置 target 为好友 poepleId)。修改后的代码如下：
   ```typescript
   ...
   ```
2. 去除 `extractTargetFromSearch` 方法的代码注释。`extractTargetFromSearch` 方法的功能是检测当前 URL 中的 target 是否指定，如果指定了好友的 peopleId 就返回好友的 peopleId 字符串。给代码如下：
...
打开 CYFS 浏览器，进入留言板前端页面，我们修改 URL 中增加 search 参数 `?target=<your friend people id>`，即可看到好友的留言板内容啦！是不是非常炫酷？
