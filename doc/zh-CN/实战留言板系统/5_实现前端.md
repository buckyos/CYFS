...
cyfs-sdk 完全支持 typescript，为Web开发者提供了云端一体的开发体验：使用type-script就能基本完成DEC App的开发
...
TODO：这里要说明一下分页读取的原理，而且这一章应该把读取放在第一步进行说明。
...
2. 调用 GlobalStateAccessStub 实例上的 list 方法分页获取到 `messages_list`下的留言对象列表
3. 遍历`messages_list`下的留言对象列表，为每一个留言对象调用 retrieveMessage 方法处理，解码后得到最终的留言对象。

---

从第 3 步我们发现，可以在前端直接从 NON 上读取到 root_state 的对象信息，使用`get_object`方法。
细心的小伙伴可能会发现，我们这里没有调用服务端的 `retrive_message` 接口去查询留言对象信息。//TODO:这个区别是很本质的，还是减少开发者的心智负担
其实，你也可以改成调用服务端的方式去读取到每一个留言对象，感兴趣的小伙伴可以试一试！
...
5. 安装完成后，在应用信息栏的右上边，有个绿色的 `进入` 标志，那就是 `前端页面的入口`，我们点击这个图标，就可以看到DEC App的入口页面了！
