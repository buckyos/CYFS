...
在这个章节，你将学到 Dec App 的前端页面所需的大部分知识并且能发布带前端页面的 Dec App 到 OOD 上，结合前面已经学习过的 Dec_Service 内容，你将能够构成一个完整的 Dec App。
...
5. 安装完成后，在应用信息栏的右上边，有个绿色的 `进入` 标志，那就是 `前端页面的入口`，我们点击这个图标，就可以看到 DEC App 的入口页面了！
