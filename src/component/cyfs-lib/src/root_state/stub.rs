...
impl GlobalStateStub {
    pub(crate) fn new(processor: GlobalStateOutputProcessorRef, target: Option<ObjectId>) -> Self {
    ...
    pub async fn create_path_op_env(&self) -> BuckyResult<PathOpEnvStub> {
        ...
        let op_env = PathOpEnvStub::new(resp, self.target.clone());
        Ok(op_env)
    }
    ...
    pub async fn create_single_op_env(&self) -> BuckyResult<SingleOpEnvStub> {
        ...
        let resp = self.processor.create_op_env(req).await?;
        let op_env = SingleOpEnvStub::new(resp, self.target.clone());
        Ok(op_env)
    }
}
...
impl SingleOpEnvStub {
    pub(crate) fn new(processor: OpEnvOutputProcessorRef, target: Option<ObjectId>) -> Self {
        Self { processor, target }
    }
