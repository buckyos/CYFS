...
pub(crate) struct NONGlobalStateMetaAclProcessor {
    global_state_meta: GlobalStateMetaService,
    next: NONInputProcessorRef,
}

impl NONGlobalStateMetaAclProcessor {
    pub fn new(
        global_state_meta: GlobalStateMetaService,
        next: NONInputProcessorRef,
    ...
    async fn check_access(
        ...
        let dec_rmeta = rmeta.get_global_state_meta(Some(global_state_common.dec_id), false).await.map_err(|e| {
            let msg = format!("get_object check rmeta but target dec rmeta not found or with error! {}, target_dec={}, {}", req_path, global_state_common.dec_id, e);
            warn!("{}", msg);
...
impl NONInputProcessor for NONGlobalStateMetaAclProcessor {
    async fn put_object(
        &self,
        ...
        if let Some(req_path) = &req.common.req_path {
            self.check_access(req_path, &source, RequestOpType::Write)
                .await?;
        }
    ...
    async fn get_object(
        ...
        if let Some(req_path) = &req.common.req_path {
            ...
            self.check_access(req_path, &source, RequestOpType::Read)
                .await?;
        }
    ...
    async fn post_object(
        ...
        if let Some(req_path) = &req.common.req_path {
            ...
            self.check_access(req_path, &source, RequestOpType::Call)
                .await?;
        }
