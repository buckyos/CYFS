...
impl TryFrom<GroupProposal> for GroupUpdateProposal {
    ...
    fn try_from(value: GroupProposal) -> Result<Self, Self::Error> {
        ...
        let (group_chunk, remain) = ChunkMeta::raw_decode(payload.as_slice()).unwrap();
        assert_eq!(remain.len(), 0);

        let to_group = Group::try_from(&group_chunk)?;

        let to_chunk_id =
            task::block_on(async { group_chunk.to_chunk().await.unwrap().calculate_id() });
        if &to_chunk_id.object_id() != param.to_chunk_id() {
            return Err(BuckyError::new(
                BuckyErrorCode::InvalidFormat,
                "the chunk in GroupUpdateProposal.body is not match with the to_chunk_id in desc.param",
            ));
        }
        ...
        let ret = Self {
            ...
            from_chunk_id: param.from_chunk_id().clone(),
            to_group,
        };
...
impl GroupUpdateProposalBuilder {
    pub fn create(
        ...
        let group_chunk = ChunkMeta::from(&to_group);
        let group_chunk_vec = {
            let len = group_chunk.raw_measure(&None).unwrap();
            let mut buf = vec![0u8; len];
            let remain = group_chunk.raw_encode(buf.as_mut_slice(), &None).unwrap();
            assert_eq!(remain.len(), 0);
            buf
        };
        ...
        let param =
            GroupUpdateGroupPropsalParam::new(target_dec_id.clone(), None, to_chunk_id.object_id());
        let param_vec = {
        ...
        Self {
            proposal,
            target_dec_id,
            from_chunk_id: from_group_chunk_id,
    ...
    pub fn build(mut self) -> GroupUpdateProposal {
        ...
        let mut from_chunk_id: Option<ObjectId> = None;
        let to_group = self.to_group.take().unwrap();
        mem::swap(&mut target_dec_id, &mut self.target_dec_id);
        mem::swap(&mut from_chunk_id, &mut self.from_chunk_id);
