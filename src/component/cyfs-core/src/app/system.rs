...
pub fn get_system_dec_app() -> &'static DecAppId {
    SYSTEM_DEC_APP.get_or_init(|| {
        SystemDecApp::gen_system_dec_id()
    })
}
...
impl SystemDecApp {
    pub fn gen_system_dec_id() -> DecAppId {
        ...
        DecApp::generate_id(owner, "cyfs-system-service").try_into().unwrap()
    }
}
...
fn test() {
    ...
    assert_eq!(id.to_string(), "9tGpLNncauC9kGhZ7GsztFvVegaKwBXoSDjkxGDHqrn6");
    println!("{}", id);
}