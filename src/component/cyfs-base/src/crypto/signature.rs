...
impl RawEncode for SignData {
    ...
    fn raw_encode<'a>(
        ...
        if buf.len() < bytes {
            let msg = format!(
                "not enough buffer for encode SignData buf, except={}, got={}",
                bytes,
                buf.len()
            );
            error!("{}", msg);
...
impl<'de> RawDecode<'de> for SignData {
...
impl RawEncode for Signature {
    fn raw_measure(&self, purpose: &Option<RawEncodePurpose>) -> Result<usize, BuckyError> {
        ...
        // sign_data: Vec<u8>
        size += self.sign.raw_measure(purpose)?;
    ...
    fn raw_encode<'a>(
        ...
        // sign_data
        let buf = self.sign.raw_encode(buf, purpose)?;
...
impl<'de> RawDecode<'de> for Signature {
    fn raw_decode(buf: &'de [u8]) -> Result<(Self, &'de [u8]), BuckyError> {
        ...
        let (sign, buf) = SignData::raw_decode(buf)?;
