...
pub(crate) struct PingManager {
    stack: WeakStack, 
    clients: RwLock<BTreeMap<DeviceId, Client>>,
}
...
impl PingManager {
    ...
    fn client_of(&self, sn_id: &DeviceId) -> Option<Client> {
        self.clients.read().unwrap().get(sn_id).clone()
    }
    ...
    pub fn reset(&self, sn_list: Vec<Device>) {
        log::info!("{} starting.", self);

        let (to_start, to_stop) = {
            let mut clients = self.clients.write().unwrap();
            let to_stop: Vec<Client> = clients.values().cloned().collect();
            clients.clear();
            ...
            for sn in sn_list {
                ...
                let client = Client::new(sn);
                clients.insert(sn_id, client.clone());
...
impl Client {
    fn new(stack: WeakStack, sn: Device) -> Client {
        let sn_id = sn.desc().device_id();
