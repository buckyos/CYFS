...
impl SingleStreamSession {
    fn new(context: &dyn DownloadContext) -> (Self, QueryContextOp) {
        let gen_id = IncreaseIdGenerator::new();
        let op_id = gen_id.generate();
        let session = Self {
            gen_id, 
            update_at: context.update_at(), 
    ...
    fn check_context(&mut self, context: &dyn DownloadContext) -> SessionOp {
        ...
        match self.trying.clone() {
            ...
            TryingSession::None => {
                if context.update_at() > self.update_at {
    ...
    fn on_query_finished(&mut self, owner: ChunkDownloader, op: &QueryContextOp, result: (LinkedList<DownloadSource<DeviceDesc>>, Timestamp)) -> SessionOp {
        ...
        if self.update_at > update_at {
            info!("{} ignore queried sources for updated, op_id={}", owner, op.op_id);
            return SessionOp::None;
        }
                    ...
                    if let StateImpl::Loading = state {
                        if finished {
                            *state = StateImpl::Finished;
                            ...
                            let (downloading, op) = SingleStreamSession::new(downloader.owner().context());
        ...
        let (speed, op) = {
            ...
            match &mut *state{
                StateImpl::Downloading(downloading) => {
                    ...
                    let op = downloading.check_context(self.owner().context());
                    (speed, op)
                }
                _ => (0, SessionOp::None)
            }
