...
pub enum DownloadSessionState2 {
    Downloading(u32), 
    Finished,
...
impl StateImpl {
    fn to_session_state(&self) -> DownloadSessionState2 {
        match self {
            Self::Interesting(_) => DownloadSessionState2::Downloading(0), 
            Self::Downloading(_) => DownloadSessionState2::Downloading(0), 
            Self::Finished(_) => DownloadSessionState2::Finished, 
            Self::Canceled(canceled) => DownloadSessionState2::Canceled(canceled.err.code()),
        }
    }
}
...
#[derive(Clone)]
pub struct DownloadSession2(Arc<SessionImpl>);

impl std::fmt::Display for DownloadSession2 {
...
impl DownloadSession2 {
    ...
    pub fn state(&self) -> DownloadSessionState2 {
    ...
    pub async fn wait_finish(&self) -> DownloadSessionState2 {
        enum NextStep {
            Wait(AbortRegistration), 
            Return(DownloadSessionState2)
        }
        let next_step = {
            ...
            match state {
                ...
                StateImpl::Finished(_) => NextStep::Return(DownloadSessionState2::Finished), 
                StateImpl::Canceled(canceled) => NextStep::Return(DownloadSessionState2::Canceled(canceled.err.code())),
            }
        };
        match next_step {
