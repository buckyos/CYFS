...
mod dump_win {
    ...
    pub unsafe fn create_dump(dir: &Path, filename: &str, full_dump: bool) {
        ...
        let dump_type = if full_dump { MINIDUMP_TYPE::MiniDumpWithFullMemory } else { MINIDUMP_TYPE::MiniDumpNormal };

        let _ = MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), file.as_raw_handle(), dump_type, NULL, NULL, NULL);
...
#[cfg(not(windows))]
mod dump_posix {
    use libc::{kill, getpid, SIGABRT};
    pub unsafe fn create_dump(dir: &Path) {
...
pub fn create_dump(dir: &Path, filename: &str, full_dump: bool) {
    #[cfg(windows)]
    unsafe {dump_win::create_dump(dir, filename, full_dump);}

    #[cfg(not(windows))]
    unsafe {dump_posix::create_dump(dir);}
}