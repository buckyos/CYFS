    ...
    SubCommand::with_name("modify")
        ...
        .arg(
            Arg::with_name("eps")
                ...
                .short("e")
                .value_delimiter(";")
                .help("new endpoint list"),
        )
        .arg(
            Arg::with_name("admins")
                .long("admins")
                .short("A")
                .value_delimiter(";")
                .help("set administrators to group. format [PeopleId:title]"),
        )
        .arg(
            Arg::with_name("add_admins")
                .long("add_admin")
                .value_delimiter(";")
                .help("append administrators to group. format [PeopleId:title]"),
        )
        .arg(
            Arg::with_name("remove_admins")
                .long("rm_admin")
                .value_delimiter(";")
                .help("remove administrators from group. format [PeopleId]"),
        )
        .arg(
            Arg::with_name("members")
                ...
                .short("m")
                .value_delimiter(";")
                .help("set members to group. format [PeopleId:title]"),
        )
        .arg(
            Arg::with_name("add_members")
                .long("add_member")
                .value_delimiter(";")
                .help("append members to group. format [PeopleId:title]"),
        )
        .arg(
            Arg::with_name("remove_members")
                .long("rm_member")
                .value_delimiter(";")
                .help("remove members from group. format [PeopleId]"),
        )
        .arg(
        ...
        .arg(
            Arg::with_name("add_oods")
                ...
                .short("o")
                .value_delimiter(";")
                .help("device id append to people or group"),
        )
        .arg(
            Arg::with_name("ood_lists")
                .long("ood_lists")
                .short("l")
                .value_delimiter(";")
                .help("device id set to people or group"),
        )
        .arg(
            Arg::with_name("name")
                .short("n")
        ...
        .arg(
            Arg::with_name("icon")
                ...
                .long("icon")
                .takes_value(true)
                .help("icon of people or group"),
        )
        .arg(
            Arg::with_name("source")
                .long("source")
                .value_delimiter(";")
                .help("add source to app, {ver}:{id}"),
        )
        .arg(
        ...
        .arg(
            Arg::with_name("app_ver")
                .long("appver")
                .takes_value(true)
                .help("app ver add to app list"),
        )
        .arg(
            Arg::with_name("app_status")
                .long("appstart")
        ...
        Ok((desc, _)) => match desc {
            ...
            AnyNamedObject::Core(obj) => match CoreObjectType::from(obj.desc().obj_type()) {
                CoreObjectType::DecApp => {
                    ...
                    if let Some(values) = matches.values_of_lossy("source") {
                        for value in &values {
                            ...
                            app.set_source(
                                sources[0].to_owned(),
                                ObjectId::from_str(sources[1]).unwrap(),
                                None,
                            );
                        }
                    }

                    app.encode_to_file(path.as_ref(), false)
                        .expect("write desc file err");
                }
                CoreObjectType::AppList => {
                    ...
                    list.encode_to_file(path.as_ref(), false)
                        .expect("write desc file err");
                }
                _ => {}
            },
...
fn modify_group_desc(group: &mut Group, matches: &ArgMatches) -> BuckyResult<()> {
    ...
    match get_group_members_from_matches(matches, "add_members") {
        Ok(additional_members) => {
            if let Some(additional_members) = additional_members {
                ...
                additional_members.into_iter().for_each(|m| {
                    members.insert(m.id, m);
                });
                group.set_members(members.into_iter().map(|(_, m)| m).collect());
            }
        }
        Err(err) => {
            log::error!("update group({}) failed for invalid member.", group_id);
    ...
    match get_group_members_from_matches(matches, "admins") {
        ...
        Err(err) => {
            log::error!(
                "update group({}) failed for invalid administrator.",
                group_id
            );
            return Err(err);
        }
    ...
    match get_group_members_from_matches(matches, "add_admins") {
        Ok(additional_admins) => {
            if let Some(additional_admins) = additional_admins {
                ...
                let org = group.check_org_body_content_mut();
                let mut admins = org.admins().clone();
                additional_admins.into_iter().for_each(|m| {
                    admins.insert(m.id, m);
                });
    ...
    if let Some(additional_oods) = get_deviceids_from_matches(matches, "add_oods") {
        ...
        additional_oods.into_iter().for_each(|id| {
            oods.insert(id);
        });
    ...
    if let Some(version) = matches.value_of("version") {
        let version = match version.parse::<u64>() {
            ...
            Err(e) => {
                let msg = format!(
                    "update group({}) failed for invalid version {}, err: {:?}",
                    group_id, version, e
                );
                log::error!("{}", msg);
