...
use cyfs_base::{
    ...
    sign_and_set_named_object_body, sign_and_set_named_object_desc,
};
use cyfs_base::{
    AnyNamedObject, FileDecoder, FileEncoder, ObjectId, ObjectLink, PrivateKey, RsaCPUObjectSigner,
    SignatureSource, StandardObject, SIGNATURE_SOURCE_REFINDEX_OWNER,
...
pub fn sign_subcommand<'a, 'b>() -> App<'a, 'b> {
    SubCommand::with_name("sign")
        ...
        .arg(
            Arg::with_name("sign_desc")
                .short("d")
                .long("sign_desc")
                .help("add sign for desc"),
        )
        .arg(
            Arg::with_name("sign_body")
                .short("b")
                .long("sign_body")
                .help("add sign for body"),
        )
        .arg(
            Arg::with_name("sign_source")
                .short("t")
                .long("sign_source")
                .takes_value(true)
...
pub async fn sign_desc(matches: &ArgMatches<'_>) {
    if let Ok((private, _)) =
        PrivateKey::decode_from_file(matches.value_of("secret").unwrap().as_ref(), &mut vec![])
    {
        if let Ok((mut obj, _)) = AnyNamedObject::decode_from_file(
            matches.value_of("desc").unwrap().as_ref(),
            &mut vec![],
        ) {
            let signer = RsaCPUObjectSigner::new(private.public(), private);
            let sign_type = matches
                .value_of("sign_source")
                .map(|str| match str {
            ...
            if matches.is_present("sign_desc") {
                if append {
                    match_any_obj_mut!(
                        ...
                        o,
                        {
                            sign_and_push_named_object_desc(&signer, o, &sign_type)
                                .await
                                .unwrap();
                            signed = true;
                        },
                        _id,
                        {
                            error!("not support sign for chunkid");
                        }
                    );
                } else {
                    match_any_obj_mut!(
                        ...
                        o,
                        {
                            sign_and_set_named_object_desc(&signer, o, &sign_type)
                                .await
                                .unwrap();
                            signed = true;
                        },
            ...
            if matches.is_present("sign_body") {
                if append {
                    match_any_obj_mut!(
                        ...
                        o,
                        {
                            sign_and_push_named_object_body(&signer, o, &sign_type)
                                .await
                                .unwrap();
                            signed = true;
                        },
                    ...
                    match_any_obj_mut!(
                        ...
                        o,
                        {
                            sign_and_set_named_object_body(&signer, o, &sign_type)
                                .await
                                .unwrap();
                            signed = true;
                        },
