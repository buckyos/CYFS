...
impl Engine256 {
    ...
    fn get_state(&self) -> (u64, &[u32; 8]) {
        (self.len, self.state.get_state())
    }
}

impl From<(u64, &[u32; 8])> for Engine256 {
    fn from(state: (u64, &[u32; 8])) -> Self {
        Self {
            len: state.0,
            buffer: Default::default(),
            state: Engine256State::new(state.1)
        }
    }
}
...
impl From<(u64, &[u32; 8])> for Sha256 {
    fn from(state: (u64, &[u32; 8])) -> Self {
        Sha256 {
            engine: Engine256::from(state)
        }
    }
}

impl Sha256 {
    pub fn get_state(&self) -> (u64, &[u32; 8]) {
