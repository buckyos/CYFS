...
async fn put_object(target_stack: &SharedCyfsStack,  dec_id: &ObjectId) {
    ...
    let ret = target_stack.router_handlers().add_handler(
    ...
    // 发起一次QA
    {
        ...
        match ret {
            Ok(resp) => {
                ...
                let resp_object = resp.object.object.unwrap();
