...
use crate::{Bench, BenchEnv, DEC_ID, sim_zone::SimZone, Stat};
use log::*;
use cyfs_base::*;
use cyfs_core::*;
use cyfs_lib::*;
...
impl RootStateBench {
    pub fn new(stack: SharedCyfsStack, target: Option<ObjectId>, stat: Arc<Stat>, run_times: usize) -> Self {
        Self {
            run_times,
            stack,
            target,
    ...
    async fn test_path_op_env_cross_dec(&self) {
...
impl Bench for RootStateBench {
    async fn bench(&self) -> BuckyResult<()> {
        info!("begin test RootStateBench...");
        let begin = std::time::Instant::now();
        self.test().await?;
        let dur = begin.elapsed();
        info!("end test RootStateBench: {:?}", dur);
        let costs = dur.as_millis() as u64;
        self.stat.write(ROOT_STATE_ALL_IN_ONE, costs).await;
