use cyfs_base::{BuckyResult, BuckyError, BuckyErrorCode, bucky_time_to_system_time};
use async_std::task;
...
const MONGO_DB_DEFAULT_NAME: &str = "perf-db";
...
impl Default for MongoConfig {
    fn default() -> Self {
        Self {
            mongo_url: "mongodb://localhost".to_owned(),
        }
    }
}

pub(crate) struct MangodbStorage{
    client: Client,
    db: Database,
    request_coll: Collection<Document>,
    action_coll: Collection<Document>,
    acc_coll: Collection<Document>,
    record_coll: Collection<Document>,
}

fn bucky_time_to_chrono_time(bucky_time: u64) -> chrono::DateTime<Utc> {
    let sys_time = bucky_time_to_system_time(bucky_time);
    sys_time.into()
}

impl MangodbStorage {
    pub async fn new(isolate: Option<&str>, config: &MongoConfig) ->BuckyResult<Self> {
        let mut client_options = ClientOptions::parse(&config.mongo_url).await.unwrap();
    ...
    fn dump_coll(coll: &Collection<Document>, coll_name: String) {
        let coll = coll.clone();
    ...
    fn is_exists_error(e: &Error) -> bool {
        if let ErrorKind::Write(WriteFailure::WriteError(e)) = e.kind.as_ref() {
            if e.code == 11000 {
                return true;
            }
        }
    ...
    async fn insert_reqs_list(&self, people_id: String, device_id: String, dec_id: String, dec_name: String, version: String, all: &HashMap<String, PerfIsolateEntity>) -> BuckyResult<()> {
        ...
        for (_k, v) in all {
            ...
            for (_k1, v1) in reqs {
                ...
                doc.insert("time", bucky_time_to_chrono_time(v1.time_range.begin));
                doc.insert("time_begin", bucky_time_to_chrono_time(v1.time_range.begin));
                doc.insert("time_end", bucky_time_to_chrono_time(v1.time_range.end));
                doc.insert("total", v1.total);
                doc.insert("success", v1.success);
                doc.insert("total_time", v1.total_time as i64);
                if let Some(total_size) = v1.total_size {
                    doc.insert("total_size", total_size as i64);
                } else {
    ...
    async fn insert_action_list(&self, people_id: String, device_id: String, dec_id: String, dec_name: String, version: String, all: &HashMap<String, PerfIsolateEntity>) -> BuckyResult<()> {
        ...
        for (_k, v) in all {
            ...
            for action in actions {
                ...
                doc.insert("time", bucky_time_to_chrono_time(action.time));
    ...
    async fn insert_acc_list(&self, people_id: String, device_id: String, dec_id: String, dec_name: String, version: String, all: &HashMap<String, PerfIsolateEntity>) -> BuckyResult<()> {
        ...
        for (_k, v) in all {
            ...
            for (_k1, v1) in accs {
                doc.insert("item_id", v1.id);
                doc.insert("time", bucky_time_to_chrono_time(v1.time_range.begin.clone()));
                doc.insert("time_begin", bucky_time_to_chrono_time(v1.time_range.begin));
                doc.insert("time_end", bucky_time_to_chrono_time(v1.time_range.end));
                doc.insert("total", v1.total);
                doc.insert("success", v1.success);
    ...
    async fn insert_record_list(&self, people_id: String, device_id: String, dec_id: String, dec_name: String, version: String, all: &HashMap<String, PerfIsolateEntity>) -> BuckyResult<()> {
        ...
        for (_k, v) in all {
            ...
            for (_k1, v1) in records {
                doc.insert("item_id", v1.id);
                doc.insert("time", bucky_time_to_chrono_time(v1.time));
                doc.insert("total", v1.total as i64);
                if let Some(total_size) = v1.total_size {
                    doc.insert("total_size", total_size as i64);
                } else {
                    doc.insert("total_size", 0);
