...
pub struct ChainCreator {
    reserved: (
        std::marker::PhantomData<StorageRef>,
        std::marker::PhantomData<dyn State>,
    ),
}
...
impl ChainCreator {
    pub fn create_chain(
        ...
        let runner_fut = Box::pin(async move {
            ...
            let mut block_body = BlockBody::new();
            {
                ...
                state
                    .create_cycle_event_table(meta_config.get_rent_cycle())
                    .await?;
                let state_hash = storage.state_hash().await?;

                info!("create state_hash:{}", state_hash.to_string());

                if config.chain_type.is_none()
                    || config.chain_type.as_ref().unwrap() == "standalone"
                {
                    ...
                    let (miner_key, _) =
                        PrivateKey::decode_from_file(miner_key_path.as_path(), &mut Vec::new())?;
                    let (miner_group, _) =
                        MinerGroup::decode_from_file(miner_group_path.as_path(), &mut Vec::new())?;
                    let (desc, _) =
                        Device::decode_from_file(miner_desc_path.as_path(), &mut Vec::new())?;
                    ...
                    let url = format!(
                        "{}/tx_full/{}",
                        config.bfc_spv_node,
                        config.sub_chain_tx.unwrap()
                    );
                    let tx_full_info = Result::<TxFullInfo, u32>::clone_from_slice(
                        http_get_request(url.as_str()).await?.as_slice(),
                    )?;
                    ...
                    if let MetaTxBody::CreateSubChainAccount(miner_group) =
                        &tx_full_info.tx.desc().content().body.get_obj()[0]
                    {
                        let miner_key_path = {
                            let path = PathBuf::from(config.miner_key_path.as_ref().unwrap());
                            if path.is_absolute() {
                        ...
                        let sub_chain_mint = SubChainMint::new(
                            miner_group.desc().calculate_id(),
                            &state,
                            &meta_config,
                ...
                let ret = BlockExecutor::execute_block(
                    &header,
                    &mut block_body,
                    &state,
                    &meta_config,
            ...
            let ret: BuckyResult<MinerRef> = if config.chain_type.is_none()
                || config.chain_type.as_ref().unwrap() == "standalone"
            {
                let chain =
                    Chain::new(output_path.to_path_buf(), Some(block), storage, None).await?;
                ...
                || config.chain_type.as_ref().unwrap() == "bft_sub"
            {
                let miner_key_path = {
    ...
    pub fn start_miner_instance(
        dir: &Path,
        new_storage: fn(path: &Path) -> StorageRef,
    ) -> BuckyResult<Arc<dyn Miner>> {
        async_std::task::block_on(async move {
            ...
            let ret: BuckyResult<Arc<dyn Miner>> = if config.chain_type.is_none()
                || config.chain_type.as_ref().unwrap() == "standalone"
            {
                ...
                || config.chain_type.as_ref().unwrap() == "bft_sub"
            {
                ...
                let miner = BFTMiner::load(
                    config.chain_type.as_ref().unwrap().to_owned(),
                    config.coinbase.clone(),
                    config.interval,
                    config.bfc_spv_node.clone(),
                    dir,
    ...
    pub fn start_chain_instance(
        _dir: &Path,
        _new_storage: fn(path: &Path) -> StorageRef,
    ) -> BuckyResult<ChainStorage> {
        unimplemented!()
    }
}
