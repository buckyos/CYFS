...
function build(prog, buildType, target, version, channel) {
    ...
    if (target === 'aarch64-linux-android') {
        ...
        child_process.execSync(`${cmd} --strip-debug target/${target}/${buildType}/${bin_name}${ext}`)
        ...
        child_process.execSync(`bash -c "${cmd} --strip-debug target/${target}/${buildType}/${bin_name}${ext}"`)
    }
    return `${bin_name}${ext}`
}
